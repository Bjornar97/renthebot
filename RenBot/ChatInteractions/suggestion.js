...
export default {
  ...
  async getSuggestion() {
    ...
    let suggestion = await suggestionsCollection.where("random", ">=", random)
        .orderBy("random", randomBit === 1 ? "asc": "desc")
        .limit(1).get();

    if (suggestion.empty) {
        suggestion = await suggestionsCollection.where("random", "<=", random)
            .orderBy("random", randomBit === 1 ? "asc": "desc")
            .limit(1).get();
    }
    ...
    return `SUGGESTION${data.user ? ' by @' + data.user: ''}: ${data.text} | Do you have a suggestion for HermitCraft 7? Go to https://renthebot.web.app/suggestion/new`;
  },
  async linkSuggestion(code, displayName) {
    try {
        const suggestionsCollection = db.collection("suggestions");
        let query = await suggestionsCollection.where("random", "=", parseInt(code, 10)).limit(1).get();
        ...
        await suggestionsCollection.doc(query.docs[0].id).update({user: displayName});
