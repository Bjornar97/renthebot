...
export default {
    ...
    switch (comArray[0]) {
      case "info":
        ...
        if (startTimes.get(name)) {
          return `TIMER ${name === "default" ? "" : name}: ${this.getTime(
            startTimes.get(name)
          )}`;
        } else {
      ...
      case "start":
        ...
        if (typeof startTime === "number") {
          return `{user} The timer ${
            name === "default" ? "" : name
          } has alredy started. To stop it, !timer stop ${
            name === "default" ? "" : " -n" + name
          }`;
        } else {
          ...
          if (calloutIndex !== -1) {
            try {
              let intervalLenght =
                parseInt(comArray[calloutIndex + 1]) * 1000 * 60;
              ...
              intervals[name] = setInterval(() => {
                let minutes = Math.round(
                  (Date.now() - startTimes.get(name)) / (1000 * 60)
                );
                say(
                  "rendogtv",
      ...
      case "stop":
        if (comArray.includes("-a")) {
          startTimes.forEach((startTime, name) => {
            ...
            say(
              "rendogtv",
              `TIMER ${name === "default" ? "" : name}: ${time} Stopped`
            );
          });
  ...
  getTime(timestamp) {
    if (timestamp > 0) {
      ...
      let mins =
        diff.getMinutes() > 9 ? diff.getMinutes() : `0${diff.getMinutes()}`;
      let secs =
        diff.getSeconds() > 9 ? diff.getSeconds() : `0${diff.getSeconds()}`;
      return `${mins}:${secs}`;
    } else {
      return "ERROR";
    }
