...
const streamdayCollection = db
  .collection("hermitcraft")
  .doc("livestreamdays")
  .collection("days");
...
export default {
  async getNextStreamDayID() {
    ...
    try {
      const streamday = await streamdayCollection
        .where("end", ">=", date)
        .limit(1)
        .get();
  ...
  async getNextStreamer() {
    ...
    let date = new Date();
    try {
      let streamer = await streamdayCollection
        .doc(streamdayid)
      ...
      let string = `${
        streamerDoc.id.charAt(0).toUpperCase() + streamerDoc.id.slice(1)
      } is up next. Starting at `;
